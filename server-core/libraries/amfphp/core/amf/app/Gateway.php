	...
	function service() {
		...
		if(isset($GLOBALS["HTTP_RAW_POST_DATA"]) && $GLOBALS["HTTP_RAW_POST_DATA"] != "")
		{
			...
			foreach($this->filters as $key => $filter)
			{
                ...
                $end = microtime(true);
                $performance[$key] = ' took ' . ($end - $start) * 1000 . ' ms ';
			}
            //debug(__FILE__, 'AMF Gateway performance: ' . print_r($performance, true));
			...
			{
			    if((!isset($this->_gzipCompressionThreshold) || strlen($output) > $this->_gzipCompressionThreshold)  &&
