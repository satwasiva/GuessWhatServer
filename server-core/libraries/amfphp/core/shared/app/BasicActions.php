...
function classLoaderAction (&$amfbody) {
    ...
    $index = 0;
    $changed_dir = false;
    foreach($amfbody->classPaths as $classPath) {
        if(!$amfbody->noExec)
        {
            ...
            if(!$fileExists)
            {
                $ex = new MessageException(E_USER_ERROR, "The class {" . $amfbody->className . "} could not be found under the class path {" . $classPath . "}" , __FILE__, __LINE__, "AMFPHP_FILE_NOT_FOUND");
            ...
            $fileIncluded = Executive::includeClass($amfbody, "./" . basename($classPath));
    ...
    // error condition -- classhpath file doesn't exist
    $ex = new MessageException(E_USER_ERROR, "The classpath folder {" . print_r($amfbody->classPaths, true) . "} does not exist. You probably misplaced your service." , __FILE__, __LINE__, "AMFPHP_CLASSPATH_NOT_FOUND");
    MessageException::throwException($amfbody, $ex);
    return false;
}
...
function adapterMap(&$results)
{
	...
	elseif(is_object($results))
	{
		...
		else
		{
            ...
            $hidden_keys = array();
            if(property_exists($results, "_hidden_client_fields")) {
                $hidden_client_fields = $results::$_hidden_client_fields;
                $hidden_keys = array_keys($hidden_client_fields);
            }

            $result_to_return = new stdClass();
            foreach($vars as $key => $value) {
                if(!in_array($key, $hidden_keys)) {
                    $result_to_return->$key = $value;
                }
            }

            // convert from absolute timestamp to delta time w.r.t now
            if (property_exists($results, "_time_delta_fields")) {
                foreach ($results::$_time_delta_fields as $td_field_name => $absolute_time_field_name) {
                    $time = strtotime($results->$absolute_time_field_name);
                    $time_delta = $time - time();
                    $result_to_return->$td_field_name = $time_delta;
                }
            }
